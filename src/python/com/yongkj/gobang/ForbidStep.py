            ...
            for j in range(15):
                if self.board[i][j] == 1:
                    if self.charge_three_forbid(i, j):
                        ...
                        s = ""
                        s += row + col
                        print("很不幸啊，你踩中了地雷般的禁手点！你踩中的三三禁手点为 (%s) ！" % (s))
                    ...
                    if self.charge_four_forbid(i, j):
                        ...
                        s = ""
                        s += row + col
                        print("很不幸啊，你踩中了地雷般的禁手点！你踩中的四四禁手点为 (%s) ！" % (s))
                    ...
                    if self.charge_long_forbid(i, j):
                        ...
                        s = ""
                        s += row + col
                        print("很不幸啊，你踩中了地雷般的禁手点！你踩中的长连禁手点为 (%s) ！" % (s))

        player_input = input("请输入你想移动的位置:")  # 玩家输入下棋的坐标
        x = ord(player_input[0].upper()) - ord("A")  # 转换坐标
        y = ord(player_input[1].upper()) - ord("A")
        self.board[x][y] = 1  # 标记为黑棋
        self.total += 1
